...
## Phase 7 — Infrastructure / Production Hardening (delivered)

| Item | Status | Notes |
|------|--------|-------|
| `Dockerfile` (multi-stage, python:3.11-slim) | ✅ built & verified | `docker build` exit 0; `app.core.migrate` entrypoint runs |
| `docker-compose.yml` | ✅ | postgres:16 + redis:7 + backend; healthchecked; backend waits for healthy DB |
| Alembic | ✅ | `migrations/env.py` (sync engine), initial migration `2ac145176d01` (19 tables) autogenerated + applied against real Postgres |
| CI workflow | ✅ | `.github/workflows/ci.yml` — test (pytest) + build (docker) jobs |
| `requirements.txt` pin fixes | ✅ | corrected non-existent pins (redis 5.4.0→5.0.8, requests 2.33.0→2.32.3, pytest 9.0.2→8.3.4, pytest-asyncio 1.3.0→0.24.0, isort 6.0.0→5.13.2, black 25.1.0→24.10.0); added `aiosqlite==0.20.0` (used by app, was undeclared) |
| `.dockerignore` | ✅ | excludes .env, test.db, tests/, caches |

**Caveat:** the live preview service still runs on systemd+SQLite (port 8000). The compose stack publishes 8000 too — bring it up on a clean host or after stopping the systemd unit. Postgres cutover is a URL switch (models are PG-compatible). OCR/voice engines (M5.1/M5.2) still need wiring.
