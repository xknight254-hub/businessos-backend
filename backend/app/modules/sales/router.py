...
from app.core.events import event_bus, Event, INVOICE_CREATED
...
async def create_sale(
    ...
    await event_bus.publish(Event(
        ...
        payload={
            ...
            "customer_id": req.customer_id, "payment_method": req.payment_method,
        },
    ))
...
async def void_sale(
    ...
    sale.status = "voided"
    for si, _ in (await repo.get_items(sale_id)):
        batch = await repo.get_inventory_batch(si.product_id)
        if batch:
